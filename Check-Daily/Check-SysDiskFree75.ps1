...
#$toemail="Nathan Keogh <nathan.keogh@ga.gov.au>"
# Current Windows MidRange Team (tighter than GAADMINS@ga.gov.au")
...
$fromemail = "SysDisk25.WIN-Script1@ga.gov.au"
...
<html><head><title>GA Daily Check - C Drive 25+ percent free (Weekly Check)</title>
<style type="text/css">
<!--
body {
...
foreach ($computer in $computers) {
	...
	#$DiskInfo=Get-WMIObject -ComputerName $computer Win32_LogicalDisk | Where-Object{$_.DeviceID -eq "C:"} | Where-Object{ ($_.freespace/$_.Size)*100 -lt $thresholdspace} | Select-Object SystemName, @{e={"{0:n2}" -f ($_.size/1gb)}}, @{e={"{0:n2}" -f ($_.freespace/1gb)}}, @{e={"{0:n2}" -f ($_.freespace/$_.size*100)}}
    ...
    # this translates to 2 columns for empty and 4 for not.
...
$CurrentSystemHTML = @"
<div id="report">
<table class="normal">$DiskInfo</table><br>
"@
} else {
...
$HTMLmessage | Out-File "C:\Health\Reports\Is_C_75_Percent\c-75-check_$todayString.html"
# Email our report out
# Only Send if 1 or more would be in the Report
if ($reportingCount -gt 0) {
send-mailmessage -from $fromemail -to $toemail -subject "C: ($reportingCount) Less Than (25%+) Free Disk (Weekly Check)" -BodyAsHTML -body $HTMLmessage -priority Low -smtpServer $server
}